...
class SciWorldGame(AbstractGame):
    ...
    def step(self, action: str) -> StepResult:
        ...
        success = self._extract_success(done=done, reward=reward, info=info, score=score)
    ...
    def _extract_action_templates(self) -> List[str]:
        ...
        if not isinstance(templates, (list, tuple)):
            return []
        return [str(template).strip() for template in templates if str(template).strip()]
    ...
    def _extract_objects(self) -> List[str]:
        ...
        if not isinstance(objects, (list, tuple)):
            return []
        return [str(obj).strip() for obj in objects if str(obj).strip()]
    ...
    def _parse_action(self, raw_action: str) -> str:
        ...
        lines = [line.strip() for line in raw_action.strip().splitlines() if line.strip()]
